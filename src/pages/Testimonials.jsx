                                ...
                                <div className="relative z-10">
                                    <div className="flex flex-col lg:flex-row lgitems-start gap-6">
                                        <div className="relative w-20 h-20 min-w-[80px]">
                                            {/* Always visible ping ring animation */}
                                            <div className="absolute inset-0 z-0 rounded-full border-2 border-orange-400 animate-ping  opacity-75" style={{ animationDuration: '1.5s' }} />

                                            {/* Outer circle with icon */}
                                            <div className={`relative z-10 w-full h-full bg-gradient-to-r from-yellow-400 to-orange-600 rounded-full p-4 shadow-lg flex items-center justify-center transition-all duration-500 ${isActive ? 'scale-110 rotate-12 shadow-2xl' : ''}`}>
                                                <FiMessageCircle className="w-full h-full text-white" />
                                            </div>

                                            {/* Inner small circle for initial */}
                                            <div className={`absolute z-20 -bottom-1 -right-1 w-8 h-8 bg-white rounded-full flex items-center justify-center shadow-lg transition-all duration-500 ${isActive ? 'scale-110 rotate-12 shadow-xl bg-gradient-to-r from-cyan-100 to-blue-100' : ''}`}>
